...
export default {
	...
	async getCategoryWithSubCategories(request, response) {
		const categoryWithSubCategories = await categoryDatamapper.findAllByPk(request.params.id, request.body);
	...
	async modifyCategory(request, response){
		const modifyCategory = await categoryDatamapper.update(request.params.id, request.body);
