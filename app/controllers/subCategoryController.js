...
export default {
	...
	async getSubCategoryByPk(request, response) {
		const subCategory = await subCategoryDatamapper.findByPk(
			request.params.id
		);
		...
		if (!subCategory) {
			...
			return response
				.status(400)
				.json({ error: "Sous-catégorie inconnue" });
		}
	...
	async getSubCategoryWithActivities(request, response) {
		const subCategoryWithActivities =
			await subCategoryDatamapper.findAllByPk(
				request.params.id,
				request.body
			);
