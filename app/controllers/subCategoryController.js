...
export default {
	...
	async getSubCategoryWithActivities(request, response) {
		const subCategoryWithActivities = await subCategoryDatamapper.findAllByPk(request.params.id, request.body);
	...
	async createSubCategory(request, response) {
		...
		if(!newSubCategory) {
			//throw new ApiError("Veuillez vérifier la saisie", { statusCode: 400 });
			return response.status(400).json({ error: "Sous-catégorie idéjà existante" });
		}
	...
	async modifySubCategory(request, response) {
		const modifySubCategory = await subCategoryDatamapper.update(request.params.id, request.body);
	...
	async deleteSubCategory(request, response) {
		const deleteSubCategory = await subCategoryDatamapper.delete(request.params.id);
